CREATE FUNCTION [dbo].[cms_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [dbo].[cms_getprimarykey_indexname]
(
...
BEGIN
	...
	SELECT CAST(data as int) FROM [cms_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [cms_splitstring_to_table](@FilteredSpecs, ',')
	...
	SELECT CAST(data as int) FROM [cms_splitstring_to_table](@AllowedUserRoleIds, ',')
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[cms_getprimarykey_indexname] ('Product') +  '] ON [nopCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[cms_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [nopCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[cms_getprimarykey_indexname] ('ProductTag') +  '] ON [nopCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	AND
	--shopping cart items
	((@OnlyWithoutShoppingCart=0) OR (NOT EXISTS(SELECT 1 FROM [ShoppingCartItem] sci with (NOLOCK) inner join [User] with (NOLOCK) on sci.[UserId]=c.[Id])))
	AND
	--guests only
