...
namespace TinyCms.Core.Infrastructure
{
    ...
    public class EngineContext
    {
        ...
        protected static IEngine CreateEngineInstance(CmsConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'TinyCms.Core.Infrastructure.IEngine' and cannot be configured in /configuration/nop/engine[@engineType] for that purpose.");
                return Activator.CreateInstance(engineType) as IEngine;
            }

            return new CmsEngine();
        }
